...
public class NativeCodec {
     ...
     * then it will return whatever we would get from {@link NativeCodec#toBytes(int)} which is
    ...
    public static final byte[] toBytes(boolean b){
        byte[] data = new byte[1];
        data[0] = (byte) (b ? 1 : 0);
        return data;
    }
    
    public static byte[][] toBytesArray(List<Integer> src) {
        byte[][] t = new byte[src.size()][];
        int index = 0;
        for (Integer v : src) {
            t[index++] = NativeCodec.toBytes(v);
        }
        return t;
    }
    ...
    /**
     * Will delegate to {@link NativeCodec#getBytes(int)} if the 'long' number is actually
     ...
     * 
     * <p>Expects a byte array of no more than {@link NativeCodec#MAX_POSITIVE_32_BIT_DIGITS} bytes in length
    ...
    /**
     * Its just like (really! :) {@link NativeCodec#toInt(byte[], int, int)} but for {@link Long} values.  Max number of digits 
     * is now {@link NativeCodec#MAX_POSITIVE_64_BIT_DIGITS}.  
     * 
     ...
     * @throws IllegalArgumentException
     */
    public static final boolean toBoolean(byte[] potentiallySignedBytes) throws IllegalArgumentException
    {
        if(null == potentiallySignedBytes) throw new IllegalArgumentException ("null input");
        return potentiallySignedBytes.length == 1 && potentiallySignedBytes[0] == 1;
    }
     ...
     * @return
     */
    public static Integer[] toIntegerArray(Collection<byte[]> bytesCollection) {
        if(bytesCollection == null) throw new IllegalArgumentException("null bytes collection");
        Integer[] r = new Integer[bytesCollection.size()];
        int index=0;
     ...
     * @return
     */
    public static List<Long> toLongList(Collection<byte[]> src) {
        if(src == null) throw new IllegalArgumentException("null input");
        List<Long> r = new ArrayList<Long>(src.size());
        for (byte[] bytes : src) {
            r.add(toLong(bytes));
        }
        return r;
     ...
     * @return
     */
    public static int[] toIntArray(Collection<byte[]> bytesCollection) {
        if(bytesCollection == null) throw new IllegalArgumentException("null bytes collection");
        int[] r = new int[bytesCollection.size()];
        int index=0;
        for (byte[] bytes : bytesCollection) {
            r[index++] = toInt(bytes);
        }
    ...
    public static List<Integer> toIntList(Collection<byte[]> bytesCollection) {
        if(bytesCollection == null) throw new IllegalArgumentException("null bytes collection");
        List<Integer> r = new ArrayList<Integer>(bytesCollection.size());
        for (byte[] bytes : bytesCollection) {
            r.add(toInt(bytes));
        }
        return r;
